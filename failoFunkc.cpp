...
void nuskaitymas_list(list<duom>& lst, int* pazsk, int& studsk, string pavadinimas) {
    //auto start = std::chrono::high_resolution_clock::now();
    int laikinas; //balo reiksme
    ifstream skaitymas;
    string buff, eil;
...
void isvedimas_i_faila(vector<duom> v, int pazsk) { //V0.2 versija
    ofstream output;
...
void naujas_skirstymas(vector<duom>&v, vector < duom>& m) {
    auto start = std::chrono::high_resolution_clock::now();
    vector<duom>::size_type i = 0;
    while (i != v.size()) {
        if (galutinis(mediana(v[i].paz), v[i].egz) && galutinis(vidurkis(v[i].paz), v[i].egz) < 5) {
            m.push_back(v[i]);
            v.erase(v.begin() + i);
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration < double>diff = end - start;
    cout << "(VEKTORIUS NAUJA VERSIJA) Skirstymas i kietus ir minkstus uztruko: " << diff.count() << "s\n";
}
void remove_if_skirstymas(vector<duom>& v, vector <duom>& m) {
    auto start = std::chrono::high_resolution_clock::now();
    vector<duom>::iterator it;
    v.erase(remove_if(v.begin(), v.end(), [&m](duom naujas) {
        if (galutinis(mediana(naujas.paz), naujas.egz) && galutinis(vidurkis(naujas.paz), naujas.egz) < 5) {
            m.push_back(naujas);
            return true;
        }
        else { return false; }
...
void naujas_skirstymas_list(list<duom>& lst, list<duom>& m) {
    auto start = std::chrono::high_resolution_clock::now();
    list<duom>::iterator IT = lst.begin();
    while (IT != lst.end())
    {
        if (galutinis(mediana((*IT).paz), (*IT).egz) && galutinis(vidurkis((*IT).paz), (*IT).egz) < 5.0)
        {
            m.push_back(*IT);
            IT = lst.erase(IT);
        }
    ...
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration < double>diff = end - start;
    cout << "(LIST NAUJA VERSIJA) Skirstymas i kietus ir minkstus uztruko: " << diff.count() << "s\n";
...
void isvedimas_i_2_failus_list(int studsk, list<duom> k, list<duom> m) {
    auto start = std::chrono::high_resolution_clock::now();
    ofstream output1, output2;
    ...
    for (list<duom>::iterator IT = k.begin(); IT != k.end(); IT++) {
        output1 << setw(20) << left << (*IT).var << setw(20) << left << (*IT).pav;
        output1 << fixed << setprecision(2) << setw(20) << left << galutinis(mediana((*IT).paz), (*IT).egz) << galutinis(vidurkis((*IT).paz), (*IT).egz) << endl;
    }
    output2.open("(list)minksti" + to_string(studsk) + ".txt");
    output2 << setw(20) << left << "Vardas" << setw(20) << left << "Pavarde" << setw(40) << left << "Galutinis(med.)/Galutinis(vid.)" << endl;
    output2 << string(80, '-') << endl;

    for (list<duom>::iterator IT = m.begin(); IT != m.end(); IT++) {
        output2 << setw(20) << left << (*IT).var << setw(20) << left << (*IT).pav;
        output2 << fixed << setprecision(2) << setw(20) << left << galutinis(mediana((*IT).paz), (*IT).egz) << galutinis(vidurkis((*IT).paz), (*IT).egz) << endl;
    }
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration < double>diff = end - start;
    cout << "Isvedimas i du failus uztruko: " << diff.count() << "s\n";
